...
for ii=1:length(trainIdx)
    i = trainIdx(ii);
    numSamp = length(fly.indices{i}) - 2;
    X(idx:idx+numSamp-1) = fly.VT(fly.indices{i}(3:end));
    j = fly.indices{i}(2:end-1);
...
for ii=1:length(trainIdx)
    i = trainIdx(ii);
    numSamp = length(fly.indices{i}) - 2;
    X(idx:idx+numSamp-1) = fly.VS(fly.indices{i}(3:end));
    j = fly.indices{i}(2:end-1);
    U(idx:idx+numSamp-1,:) = [cos(fly.pos_o(j)), sin(fly.pos_o(j)), ...
        fly.VT(j), fly.VS(j), fly.VR(j), fly.stim_RT(j,1), fly.stim_RT(j,2)];
    idx = idx + numSamp;
...
for ii=1:length(trainIdx)
    i = trainIdx(ii);
    numSamp = length(fly.indices{i}) - 2;
    X(idx:idx+numSamp-1) = fly.VR(fly.indices{i}(3:end));
    j = fly.indices{i}(2:end-1);
    U(idx:idx+numSamp-1,:) = [cos(fly.pos_o(j)), sin(fly.pos_o(j)), ...
        fly.VT(j), fly.VS(j), fly.VR(j), fly.stim_RT(j,1), fly.stim_RT(j,2)];
    idx = idx + numSamp;
